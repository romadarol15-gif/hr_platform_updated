...
def profile(request, employee_id=None):
    ...
    # Админы/бухгалтеры могут редактировать всё, включая ФИО, роль, должность, отдел
    if can_edit and is_admin_or_accountant:
        FormClass = EmployeeFullForm
    elif can_edit:
        # Обычные сотрудники не могут редактировать ФИО, роль, должность, отдел
        FormClass = EmployeeRestrictedForm
    else:
        # Для просмотра используем ограниченную форму
        FormClass = EmployeeRestrictedForm
